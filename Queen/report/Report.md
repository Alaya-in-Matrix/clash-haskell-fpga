...
* Last Modified:2015年11月07日 星期六 20时55分15秒 六
...
很多有用的高阶函数都围绕列表定义，如`map`， `filter`， `zipWith`，`fold`等，这几个高阶函数在N皇后问题实现时经常使用，因此事先介绍一下。不过，这些高阶函数并非智能定义在列表上，它们都是多态函数，例如，`fold`不仅可以对列表做规约，也可以对其他数据结构，例如tree等进行规约，而下面只介绍它们在列表上的实现。

`map`函数把一个列表映射到另一个列表，其函数类型签名与在列表上的大致实现如下，它接受一个映射函数，和一个列表，并对列表中的每一个元素应用映射函数，返回的生成的新列表。
